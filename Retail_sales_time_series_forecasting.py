...
def train_lstm(train: pd.Series, n_steps: int):
    ...
    model = Sequential([
        LSTM(CONFIG["LSTM_UNITS"], return_sequences=True, input_shape=(n_steps, 1)),
        Dropout(0.2),
    ...
    forecast = pd.Series(forecast, index=train.index[-CONFIG["FORECAST_HORIZON"]:])
...
def evaluate_forecast(true: pd.Series, pred: pd.Series) -> dict:
    ...
    if np.any(non_zero_mask):
        ...
        mape = np.nan  # If all y_true values are 0
...
def main():
    ...
    with tab2:
        ...
        models = {"SARIMA": train_sarima, "Prophet": train_prophet, "LSTM": lambda x: train_lstm(x, CONFIG["LSTM_N_STEPS"])}
        rows = []
